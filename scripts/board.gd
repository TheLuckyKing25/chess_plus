...
func _on_piece_clicked(new_selected_piece: Node3D) -> void:
	if selected_piece:
		...
		if new_selected_piece.piece_state(PieceStateFlag.SELECTED,Callable(self,"flag_is_enabled")):
			new_selected_piece.piece_state(PieceStateFlag.SELECTED,Callable(self,"unset_flag"))
		...
		# unselect the current piece and select the new piece
			...
			selected_piece.piece_state(PieceStateFlag.SELECTED,Callable(self,"unset_flag"))
			get_tree().call_group("Tile","clear_move_states")
			new_selected_piece.piece_state(PieceStateFlag.SELECTED,Callable(self,"set_flag"))
...
func promote(promotion: PawnPromotion):
	...
	match promotion:
		PawnPromotion.ROOK:
			change_piece_resources(selected_piece,PieceType.ROOK)
			selected_piece.add_to_group("Rook")
		PawnPromotion.BISHOP: 
			change_piece_resources(selected_piece,PieceType.BISHOP)
			selected_piece.add_to_group("Bishop")
		PawnPromotion.KNIGHT:
			change_piece_resources(selected_piece,PieceType.KNIGHT)
			selected_piece.add_to_group("Knight")
		PawnPromotion.QUEEN:
			change_piece_resources(selected_piece,PieceType.QUEEN)
