...
class Printer:
	...
	static func print_state_enter(state_name:String):
		if debug_print_switch.state_enter:
			print_rich("[b][color=web_green]Entered[/color]: [/b]",state_name)


	static func print_state_exit(state_name:String):
		if debug_print_switch.state_exit:
			print_rich("[b][color=brown]Exited[/color]: [/b]",state_name)


	static func print_class_log(classname:String, text:String, classname_color:String = "deep_sky_blue"):
	...
	static func print_pretty(value, detailed:bool = true, _indent:int = 0):
		var string: String = ""
		if value is Array:
	...
	static func _generate_array_string(value:Array, detailed:bool = true, _indent:int = 0) -> String:
	...
	static func _generate_dict_string(value:Dictionary, detailed:bool = true, _indent:int = 0) -> String:
		const MAX_COMPACTING_SIZE: int = 10
		var string: String = "{"
		var array_dict_type_filter: Callable = func(item): return not typeof(item) in [TYPE_ARRAY,TYPE_DICTIONARY]
